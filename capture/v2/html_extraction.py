...
async def _capture_html_media_and_form_attrs(page, capture_data) -> None:
    ...
    ASU's hero ``<video autoplay muted loop>`` reported every attribute
    ...
    # key) or duplicate entries appear in the output. ASU's six <ul> nodes
    ...
    landmark_items = await page.evaluate(r"""() => {""" + SELECTOR_HELPER + r"""
        ...
        // name they're generic — not landmarks. ASU's <section

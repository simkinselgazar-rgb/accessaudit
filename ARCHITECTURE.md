...
└── {review_id}/          # e.g. 20260512_143022_abc123
    ...
    │   │   └── ...
    │   └── results.json   # All TestResult objects merged
    ├── report/
    │   ├── report.pdf
    │   ├── report.docx
    │   └── report_data.json
...
| `POST` | `/api/start` | Create and queue a new review |
| `GET` | `/api/reviews` | List all reviews |
| `GET` | `/api/review/{id}` | Get review details + results |
| `GET` | `/api/review/{id}/status` | Lightweight status poll |
| `POST` | `/api/review/{id}/cancel` | Cancel a running review |
| `DELETE` | `/api/review/{id}` | Delete a review |
| `GET` | `/api/review/{id}/download/{format}` | Download report (pdf/docx/xlsx) |
